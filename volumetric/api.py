...
class AutomagicApi:
    ...
    ) -> Generator[interfaces.automagic.AutomagicInterface, None, None]:
    ...
    def get_requirements(self):
        ...
        for amagic in self.get_automagics():
            for req in amagic.get_requirements():
                if isinstance(
                    req,
                    (configuration.SimpleTypeRequirement, requirements.ListRequirement),
                ):
                    automagic_config_path = interfaces.configuration.path_join(
    ...
    def create_job(self, plugin, automagics, global_config, plugin_config):
        ...
        for automagic_req in AutomagicApi().get_requirements():
            if automagic_req["type"] == "ListRequirement":
                if not isinstance(global_config.get(automagic_req["name"], []), list):
                    global_config[automagic_req["name"]] = [
                        global_config[automagic_req["name"]]
                    ]
        for req in self.get_requirements(plugin):
            ...
            if req["type"] == "BooleanRequirement":
                plugin_config[short_req_name] = bool(
                    plugin_config.get(short_req_name, req["default"])
                )
        ...
        hash = hashlib.sha1(
            bytes(json.dumps(job, sort_keys=True), "latin-1")
        ).hexdigest()
    ...
    def run_job(self, job_id):
        ...
        def generator():
            ...
            if job["result"] is None:
                ...
                ctx.config.splice(
                    plugin_config_path, HierarchicalDict(job["plugin_config"])
                )
                    ...
                    try:
                        ...
                        if not job["running"]:
                            yield respond(
                                {
                                    "type": "error",
                                    "data": {"message": "Job no longer running"},
                                }
                            )
                            return
                        if (
                            progress["type"] == "finished"
                            or progress["type"] == "error"
                        ):
...
def generate_plugin(automagics, ctx, plugin, plugin_config_path, progress_queue):
    def progress_callback(value, message=None):
        progress_queue.put(
            {"type": "progress", "data": {"value": value, "message": message}}
        )

    def visit(node, accumulator):
        accumulator.put(
            {"type": "partial-output", "data": [node.path_depth] + list(node.values)}
        )
        return accumulator
    ...
    try:
        constructed = framework.plugins.construct_plugin(
            ctx, automagics, plugin, "plugins", progress_callback, file_handler
        )
        progress_queue.put(
            {"type": "config", "data": dict(constructed.build_configuration())}
        )
        ...
        progress_queue.put(
            {
                "type": "columns",
                "data": [("depth", "int")]
                + [(column.name, column.type.__name__) for column in result.columns],
            }
        )
        ...
        progress_queue.put(
            {
                "type": "error",
                "data": {
                    "message": "Unsatisfied requirements: {}".format(excp.unsatisfied)
                },
            }
        )
        return None
        ...
        progress_queue.put(
            {"type": "error", "data": {"message": "Exception: {}".format(str(excp))}}
        )
        vollog.debug("\n\nEXCEPTION\n{}".format("\n".join(fulltrace)))
        return None
    ...
    progress_queue.put(
        {"type": "finished", "data": {"message": "Complete"}, "result": result}
    )
...
class ResultsApi:
    ...
    def get(
        ...
        if job.get("cached_results", {}).get(
            (job_id, parent_row_id, sort_property, sort_direction)
        ):
            result_list = job.get("cached_results", {})[
                (job_id, parent_row_id, sort_property, sort_direction)
            ]
        else:
            result = job["result"]
        ...
        return {
            ...
            "results": result_list[index : index + page_size],
        }
    ...
    def metadata(self, job_id):
        ...
        if not job.get("result", None):
            return None
        return {
            "size": job["result"].row_count,
            "columns": [
                {"name": column.name, "type": column.type.__name__}
...
class JobsApi:
    ...
    def list(self):
        ...
        for job_id in jobs:
            result += [
                {"job_id": job_id, "running": jobs[job_id].get("running", False)}
            ]
        return result
